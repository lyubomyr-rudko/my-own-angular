...
/* global Scope: false, describe: false, it: false, expect: false, jasmine: false, beforeEach: false, _: false, setTimeout: false */
'use strict';
describe("Scope", function() {
  ...
  describe('digest', function () {
    ...
    it("calls listener with new value as old value the first time", function() {
      ...
      // set the watcher
      scope.$watch(function (scope) { return scope.someValue; }, function (newValue, oldValue, scope) { oldValueCopy = oldValue; });
      scope.someValue = 123;

      scope.$digest();

      // expect the count 
      expect(oldValueCopy).toBe(123);
    });
    ...
    it("ends the digest when the last watch is clean", function() { 
      ...
      _.times(100, function(i) { 
        scope.$watch(function(scope) { watchExecutions++; return scope.array[i]; }); 
      });
    ...
    it("compares based on value if enabled", function() {
      ...
      scope.$watch(function (scope) { return scope.aValue; }, function (newValue, oldValue, scope) { scope.counter++; }, true);
    ...
    it("correctly handles NaNs", function() { 
      ...
      scope.$watch(function(scope) { return scope.number; }, function(newValue, oldValue, scope) { scope.counter++; });
      scope.$digest();
      expect(scope.counter).toBe(1);
      scope.$digest();
      expect(scope.counter).toBe(1);
    ...
    it("passes the second $eval argument straight through", function() { scope.aValue = 42;
      var result = scope.$eval(function(scope, arg) { return scope.aValue + arg; }, 2);
    ...
    it("executes $evalAsync'ed functions even when not dirty", function() {
      ...
      scope.$digest(); 
      expect(scope.asyncEvaluatedCount).toBe(2);
    });
    ...
    it("has a $$phase field whose value is the current digest phase", function() { 
      ...
      scope.$apply(function(scope) { scope.phaseInApplyFunction = scope.$$phase; });
    ...
    it('coalesces many calls to $applyAsync', function(done) { 
      ...
      setTimeout(function() {
        expect(scope.counter).toBe(2); // first digest - watch is executed twice
        done();
      }, 50);
    });
    ...
    it("catches exceptions in $applyAsync", function(done) {
      ...
      expect(scope.applied).toBe(undefined);
      setTimeout(function() {
